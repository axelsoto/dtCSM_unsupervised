function [machine, err] = train_real_rbm(X, h, eta, max_iter, weight_cost)
...
% Maastricht University, 2008
    ...
    [n, v] = size(X);
    batch_size = 100;
    machine.W = randn(v, h, 'single') * 0.01;
    machine.bias_upW = zeros(1, h, 'single');
    machine.bias_downW = zeros(1, v, 'single');
    deltaW = zeros(v, h, 'single');
    deltaBias_upW = zeros(1, h, 'single');
    deltaBias_downW = zeros(1, v, 'single');
    ...
    for iter=1:max_iter
        ...
        for batch=1:batch_size:n          
            if batch + batch_size <= n
                ...
                vis1 = single(X(ind(batch:min([batch + batch_size - 1 n])),:));
                ...
                %vis2 = vis2 + randn(size(vis2));

                % Sample states from visible nodes
		        %vis_states = vis2 + randn(size(vis2));
        ...
        siz = min([n 5000]);
        %err = compute_recon_err(machine, X(1:siz,:),1);
        disp(['Iteration ' num2str(iter) ' (rec. error = ' num2str(err ./ n) ')...']);
    end
    disp(' ');
